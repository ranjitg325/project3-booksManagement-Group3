...
try {
    ...
    let isValidPass= /^(?=.*[A-Za-z])(?=.*\d)(?=.*[@$!%*#?&])[A-Za-z\d@$!%*#?&]{8,15}$/.test(password)
    if(!isValidPass){
        return res.status(400).send({ status: false, msg: "please provide an alphanumeric password including a special character between length 8 to 15" })
    }
